...
### 15. 이메일 발송 문제 완전 해결 (2025-11-22)

**사건 개요**: 2025년 10월 31일부터 시작된 Daum/Naver 이메일 발송 실패가 도메인별 발신자 선택으로 완전 해결됨

**핵심 교훈**:
> **메일 제공업체별로 신뢰하는 발신자 도메인이 다름. 도메인별 맞춤 발신자 전략이 정답**

**문제의 근본 원인**:
- Daum: `noreply@aed.pics` 차단, `noreply@nmc.or.kr` 허용
- Naver: `noreply@nmc.or.kr` 차단, `noreply@aed.pics` 허용
- DMARC/SPF 정책 불일치가 핵심

**해결책 (검증 완료)**:
```typescript
// 도메인별 발신자 선택
if (domain === 'naver.com') {
  senderEmail = 'noreply@aed.pics';  // Naver 성공
} else if (domain === 'daum.net' || domain === 'hanmail.net') {
  senderEmail = 'noreply@nmc.or.kr';  // Daum 성공
} else if (domain === 'nmc.or.kr') {
  senderEmail = 'noreply@nmc.or.kr';  // 같은 도메인
} else {
  senderEmail = 'noreply@aed.pics';  // 기본값
}
```

**실패한 시도들**:
1. 스마트 발신자 선택 (실패율 기반 로테이션) - 효과 없음
2. 새 발신자 추가 (admin@aed.pics) - 동일하게 차단됨
3. 발신자 고정 (한 개만 사용) - 특정 도메인 실패

**검증 결과 (NCP 콘솔)**:
- Daum 발송: ✅ 성공 (요청 ID: 20251122000002804903)
- Naver 발송: ✅ 성공 (요청 ID: 20251122000003089104)
- NMC 발송: ✅ 성공 (요청 ID: 20251122000003366701)

**교훈**:
1. NCP Block List는 수신자 주소 기반 (발신자 변경으로 우회 불가)
2. DMARC 정렬(alignment)이 핵심 (도메인 간 신뢰 관계)
3. 추측 대신 실제 테스트로 검증 (NCP 콘솔 로그가 진실)

**관련 문서**:
- [완전 해결 분석](docs/troubleshooting/EMAIL_SENDING_COMPLETE_ANALYSIS_2025-11-22.md)
...
**마지막 업데이트**: 2025-11-22
**문서 버전**: 2.8.0
**주요 변경**: 이메일 발송 문제 완전 해결 (교훈 15번) - 도메인별 발신자 전략 성공
