...
### 15. 스마트 발신자 선택 시스템 실패 (2025-11-21 ~ 2025-11-22)

**사건 개요**: 이메일 Daum 차단 문제를 해결하기 위해 "스마트 발신자 선택(동적 로테이션)" 시스템을 활성화했으나, 실제 배포 후 완전히 실패함.

**핵심 교훈**:
> **발신자 변경 != 차단 해제. NCP 자동 보안 정책(Block List)은 발신자 로테이션으로 우회 불가능**

**발생한 일**:

1. **초기 가정 (잘못됨)**:
   ```
   Daum 발송 문제 → 특정 발신자가 차단됨
   해결책 → 두 발신자 중 하나는 성공할 것 → 로테이션 시스템 구축
   ```

2. **현실**:
   ```
   Daum 발송 문제 → NCP의 자동 보안 정책이 Daum 주소를 Block List에 추가
   발신자 변경 → 아무 효과 없음 (차단은 주소 기반, 발신자 기반이 아님)
   결과 → 모든 발신자가 실패
   ```

3. **실제 배포 결과**:
   - noreply@aed.pics: 40% 성공 (2/5, 하지만 최근 시도는 모두 실패)
   - noreply@nmc.or.kr: 0% 성공 (0/4)
   - **결론**: 둘 다 차단됨

**핵심 문제점**:

1. **잘못된 가정**
   - "두 발신자 중 하나는 항상 성공할 것이다" ← 아님
   - "1시간마다 캐시 리셋 = 새로운 기회" ← 아님 (NCP가 자동 재추가)

2. **로그와 실제 상태 불일치**
   - 개발서버 로그: "Email sending success recorded" ✓
   - NCP 콘솔: "발송실패" ✗
   - 원인: NCP API가 요청을 수락했으나, 이후 자동 보안 정책으로 거부

3. **근본 원인은 NCP 정책**
   - 개발자가 제어할 수 없음
   - 발신자 변경으로 우회 불가능
   - NCP 기술 지원팀의 화이트리스트 등록 필수

**교훈**:

1. **발신자 로테이션은 만능이 아니다**
   - 발신자 차단 문제는 로테이션으로 해결 가능
   - 하지만 주소 차단(Block List)은 발신자 변경으로 우회 불가능

2. **시스템 계획 시 검증이 필수**
   - 가정만으로 설계하지 말 것
   - "둘 다 실패할 수 있다"는 최악의 시나리오 고려
   - 실제 NCP 정책 문서 확인

3. **외부 서비스의 제어 범위를 이해하라**
   - NCP의 자동 보안 정책: 개발자 제어 불가능
   - Block List에 추가되면: 발신자 변경은 무의미
   - 해결책: NCP 기술 지원팀

**앞으로 스마트 발신자 선택을 고려할 때**:

절대 하지 말 것:
- [ ] 다시 시도하기 (같은 이유로 실패함)
- [ ] 추가 도메인 테스트 (Daum 같은 문제 발생 가능)

해야 할 것:
- [ ] NCP 콘솔 Block List 확인
- [ ] NCP 기술 지원팀에 화이트리스트 등록 요청
- [ ] 다른 발송 서비스(AWS SES, SendGrid) 평가

**관련 문서**:
- [스마트 발신자 선택 시스템 실패 분석](docs/troubleshooting/SMART_SENDER_FAILURE_ANALYSIS_2025-11-22.md) - 상세 데이터 및 분석
- [스마트 발신자 선택 시스템 활성화 계획 (실패)](docs/troubleshooting/SMART_SENDER_ACTIVATION_PLAN_2025-11-21.md) - 실패한 계획 (참고용)

**키워드** (향후 검색용):
```
- Smart sender selector failure 스마트 발신자 선택 실패
