...
BEGIN
  IF p_query_criteria = 'jurisdiction' THEN
    ...
    SELECT
      ...
      COUNT(CASE WHEN ad.battery_expiry_date < CURRENT_DATE OR ad.patch_expiry_date < CURRENT_DATE THEN 1 END)::BIGINT AS expired_count,
      COUNT(CASE WHEN
        (ad.battery_expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days')
        OR (ad.patch_expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days')
        THEN 1 END)::BIGINT AS expiring_soon_count,
      COUNT(CASE WHEN NOT (ad.display_allowed ILIKE '%허용%' OR ad.display_allowed IN ('Y','y','YES','Yes')) THEN 1 END)::BIGINT AS hidden_count,
      0::BIGINT AS with_sensitive_data_count
    FROM aed_data ad
    LEFT JOIN organizations hc ON hc.name = ad.jurisdiction_health_center
    WHERE
      hc.id IS NOT NULL
      AND (
        (p_region_codes IS NULL)
        OR hc.region_code = ANY(p_region_codes)
        OR ad.sido = ANY(ARRAY(
          SELECT rm.label FROM (VALUES
            ('KR', '중앙'),
            ('SEO', '서울특별시'),
