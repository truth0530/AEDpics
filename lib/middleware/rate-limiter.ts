 ...
 *
 * ioredis를 사용한 요청 제한 (Naver Cloud Platform Redis)
 * - IP 기반 rate limiting
...
export async function checkRateLimit(
  ...
  if (!redis) {
    console.log('[Rate Limiter] Redis not configured, allowing all requests');
    return {
      success: true,
  ...
  try {
    ...
    const currentCount = currentCountStr ? parseInt(currentCountStr, 10) : null;
...
export function getIdentifier(request: NextRequest): string {
  // Naver Cloud Platform에서도 x-forwarded-for 헤더 사용
