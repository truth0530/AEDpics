...
/**
 * TNMS 기반 전체 매칭 실행 스크립트
 * target_list_2025 (29,295건)와 aed_data (82,104건) 자동 매칭
 */

import { PrismaClient } from '@prisma/client';
import { textNormalizer } from '../lib/services/tnms/text-normalizer.js';
import { addressNormalizer } from '../lib/services/tnms/address-normalizer.js';
...
async function findBestMatch(target, aedCandidates) {
  // Target 정규화
  const targetNameResult = await textNormalizer.normalize(target.institution_name || '');
  const targetNormalizedName = targetNameResult.normalized;

  let targetNormalizedAddress = '';
  if (target.road_address || target.lot_address) {
    const targetAddrResult = await addressNormalizer.normalize(
      target.road_address || target.lot_address
    );
    targetNormalizedAddress = targetAddrResult.road_address || targetAddrResult.lot_address;
  }
  ...
  for (const aed of aedCandidates) {
    // AED 정규화
    const aedNameResult = await textNormalizer.normalize(aed.installation_institution || '');
    const aedNormalizedName = aedNameResult.normalized;
    ...
    if (targetNormalizedAddress && aed.installation_address) {
      const aedAddrResult = await addressNormalizer.normalize(aed.installation_address);
      const aedNormalizedAddress = aedAddrResult.road_address || aedAddrResult.lot_address;
    ...
    if (totalScore > bestScore) {
      ...
      bestRules = [
        ...targetNameResult.signals.filter(s => s.applied).map(s => s.rule_name),
        ...aedNameResult.signals.filter(s => s.applied).map(s => s.rule_name)
      ];
    }
  }
  ...
  return {
    match: bestMatch,
    score: bestScore,
    rules: [...new Set(bestRules)], // 중복 제거
    targetNormalizedName
  };
}

async function executeTnmsMatching() {
  console.log('=== TNMS 전체 매칭 시작 ===');
  ...
  try {
    ...
    for (const sido of sidoList) {
      ...
      console.log(`  ${sido}: ${aedBySido[sido].length}건 AED 데이터 로드`);
    }
    ...
    for (let i = 0; i < targets.length; i += BATCH_SIZE) {
      ...
      for (const target of batch) {
        ...
        const matchingRecord = {
          ...
          matching_rules: result.rules,
          match_type: 'auto',

          updated_by: 'tnms_batch_matching'
        };
