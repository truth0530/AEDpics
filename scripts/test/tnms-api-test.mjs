 ...
 * Usage: node scripts/test/tnms-api-test.mjs
 */
...
async function testUnauthenticated(testName, method, endpoint, body) {
  try {
    console.log(`\nTesting: ${testName}`);
...
setTimeout(() => {
  makeRequest('GET', '/api/health').then(
    ...
    () => {
      ...
      console.error('\nExample:');
      console.error('  API_BASE=http://localhost:3000 node scripts/test/tnms-api-test.mjs');
      process.exit(1);
    }
  );
}, 100);
