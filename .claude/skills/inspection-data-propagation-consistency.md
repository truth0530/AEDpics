...
4. **대분류/중분류/소분류** (Categories) - 3단계 선택
5. **GPS 위도** (Latitude) - 숫자
6. **GPS 경도** (Longitude) - 숫자
7. **주소** (Address) - 텍스트
8. **설치위치** (Installation Location) - 텍스트
9. **설치 위치 접근 허용 범위** (Access Range) - 선택
10. **사용 가능 시간** (Operating Hours) - 시간 범위

### 그룹 2: 장비 정보 (DeviceInfoStep)
11. **제조사** (Manufacturer) - 텍스트
12. **모델명** (Model Name) - 텍스트
13. **제조번호/시리얼번호** (Serial Number) - 텍스트
14. **배터리 유효기간** (Battery Expiry) - 날짜
15. **패드 유효기간** (Pad Expiry) - 날짜
16. **제조일자 본체** (Manufacturing Date) - 날짜
17. **장비정상 작동 여부** (Equipment Operation) - 선택/체크
18. **사진 업로드** (Photo Capture) - 파일

### 그룹 3: 보관함 체크리스트 (StorageChecklistStep)
19. **보관함 형태** (Storage Type) - 선택
20. **도난경보장치 작동 여부** (Theft Alarm) - 선택/체크
21. **보관함 각종 안내문구 표시** (Signage in Storage) - 체크리스트
22. **비상연락망 표시 여부** (Emergency Contact Display) - 선택/체크
23. **심폐소생술 방법 안내책자/그림 여부** (CPR Guide) - 선택/체크
24. **패드 및 배터리 유효기간 표시 여부** (Expiry Display) - 선택/체크
25. **안내표지 설치** (Information Signs) - 선택/체크

### 그룹 4: 관리자 교육 (ManagerEducationStep)
26. **관리책임자 교육 이수 현황** (Manager Training) - 선택/체크
27. **보건복지부 재난의료대응과로 전달할 사항** (Report Items) - 텍스트
...
**검증 매트릭스 (25개 항목 체크리스트)**:

#### BasicInfoStep (10개 필드)
```
[ ] 1. 관리책임자: "김철수" → "이영희"
  ✅ isChanged = true
  ✅ 버튼 색상 노란색
...
[ ] 4. 대분류/중분류/소분류: 모두 변경
  ✅ 3단계 모두 isChanged = true
  ✅ 계층적 선택 유지

[ ] 5. GPS 위도: "37.4979" → "37.5000"
  ✅ isChanged = true
  ✅ 숫자 형식 검증

[ ] 6. GPS 경도: "127.0276" → "127.0300"
  ✅ isChanged = true
  ✅ 숫자 형식 검증

[ ] 7. 주소: "서울 강서구 공항대로" → "부산 중구 중앙대로"
  ✅ isChanged = true
  ✅ 텍스트 변경 감지
...
[ ] 9. 접근 허용 범위: "도보 가능" → "차량만 가능"
  ✅ isChanged = true
  ✅ 접근성 옵션 변경

[ ] 10. 사용 가능 시간: "09:00-18:00" → "24시간"
  ✅ isChanged = true
  ✅ 시간 범위 변경
```
...
[ ] 12. 모델명: "HeartStart OnSite" → "Powerheart G5"
  ✅ isChanged = true

[ ] 13. 시리얼번호: "SN-2020-001" → "SN-2020-002"
  ✅ isChanged = true

[ ] 14. 배터리 유효기간: "2025-03-31" → "2025-06-30"
  ✅ isChanged = true
  ✅ 만료 여부 자동 판정
  ✅ 조치계획 필드 활성화/비활성화

[ ] 15. 패드 유효기간: "2024-12-31" → "2025-12-31"
  ✅ isChanged = true
  ✅ 패드 상태 변경

[ ] 16. 제조일자: "2020-06-15" → "2020-08-20"
  ✅ isChanged = true
...
2. GPS 좌표 변경: (37.4979, 127.0276) → (37.5000, 127.0300)
   → Summary: 지도 마커 위치 업데이트 ✅
   → Summary: 로드뷰 업데이트 ✅

3. 주소 변경: "서울 강서구" → "부산 중구"
   → Summary: 새 주소 표시 ✅
   → Summary: 지도 이동 ✅

DeviceInfoStep 수정 시나리오:
4. 배터리 유효기간 "2025-03-31" → "2025-06-30"
   → Summary: 배터리 정보 "2025-06-30 (유효)" 표시 ✅
   → Summary: 조치계획 필드 숨김 ✅

5. 배터리 "2025-06-30" → "2025-02-28"
...
- [ ] 보관함 체크리스트 체크 개수 변경 → 점수 자동 계산 (4점 배터리+패드, 1점 안내, 1점 경보 = 6점 만점)
- [ ] 장비 상태 변경 → 종합 점검 결과 자동 계산
