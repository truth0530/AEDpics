...
- **편집 허용 범위**: `inspection_assignments.status`가 `completed`여도 메모, 스케줄 설명 등 비상태 필드는 수정 가능하지만 상태·담당자(`assigned_to`, `assignment_scope`) 변경은 금지한다. 재점검이 필요하면 새 할당을 생성한다.
- **삭제(취소) 권한**: 마스터 관리자(`role = master`)는 모든 상태의 일정을 취소(내부적으로 `status='cancelled'`)할 수 있다. 그 외 사용자는 본인이 생성한 `pending` 일정만 취소 가능하며 `completed` 일정은 삭제할 수 없다.
- **용어 정렬**: UI/문서에서는 “삭제” 대신 “일정 취소” 또는 “기록 제거(취소 처리)”로 표기하여 물리 삭제가 아닌 상태 전환임을 명시한다. 감사 로그에는 취소 사유와 계정을 함께 기록한다.
